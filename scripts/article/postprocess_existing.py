...
def shifted(output: Path) -> None:
    from scripts.article import run_final_shifted_exponential as runner
...
def holten(output: Path) -> None:
    from scripts.article import run_final_holten_h4 as runner
...
def ploemeur(output: Path) -> None:
    from scripts.article import run_ploemeur_shifted_exponential_final as runner
...
def report() -> None:
    from scripts.article.build_article_non_ploemeur_report import build
...
def main() -> int:
    ...
    location.add_argument(
        ...
        type=Path,
        help=(
            "Existing root produced by scripts.article.reproduce_article; the case "
