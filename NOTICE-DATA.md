...
PyAges source repository and Python package. It does not replace the citations
embedded in individual CSV, YAML, text, or spreadsheet files.

The PyAges software is licensed under CeCILL 2.1. That software license does not
...
| `data_core/data_tracer/sf6/recharge.csv` | NOAA Global Monitoring Laboratory SF6 values from 1995 onward, sampled on a semi-annual grid, with the legacy PyAges/TracerLPM reconstruction retained before 1995. The current file provenance does not identify direct AGAGE-derived values. Citation: Dutton et al. (2023), <https://doi.org/10.15138/TQ02-ZX42>. | Cite NOAA GML and the dataset DOI for the observational portion and retain the PyAges/TracerLPM provenance note for the pre-1995 reconstruction. |
| `data_core/data_tracer/3H/recharge.csv` | Tritium precipitation history attributed in the file header to the IAEA/WMO Global Network of Isotopes in Precipitation (GNIP/WISER), <https://nucleus.iaea.org/Pages/GNIPR.aspx>. | Cite the IAEA GNIP database. Users reusing the data outside PyAges must check the current IAEA data-use terms. |
...
| `data_core/data_tracer/kr85/recharge.csv` | Simplified annual PyAges input function based on published Northern Hemisphere atmospheric 85Kr measurements, with Ahlswede et al. (2013) and Bollhöfer et al. (2019) retained as primary literature references. An openly licensed reference compilation is Kersting et al. (2020), *Atmospheric krypton-85 activity concentrations*, Mendeley Data v1, <https://doi.org/10.17632/p32bmw6rgs.1>. The PyAges annual curve is not asserted to reproduce that compilation record-for-record. | Project redistribution of the PyAges input curve is confirmed. The Kersting et al. reference dataset is CC BY 4.0 and should be cited when reused directly. |
| Other tracer inputs under `data_core/data_tracer/` (`39Ar`, `Li`, `NO3`, and `SO4`) | Scientific constants, illustrative recharge histories, or literature-derived inputs. Citations and model assumptions are recorded where available in file headers and tracer YAML metadata. | Project redistribution confirmed. These files require scientific-source verification before reuse as authoritative observational datasets. |
...
- NOAA GML combined SF6 record: <https://gml.noaa.gov/hats/combined/SF6.html>
- Kersting et al. (2020) atmospheric 85Kr compilation: <https://doi.org/10.17632/p32bmw6rgs.1>
...
Redistribution of these source and normalized files with the PyAges repository
was confirmed by the project maintainer on 19 August 2026. Reusers should cite
the PyAges project and the **Ploemeur-Guidel observatory, SNO H+ / OZCAR**, whose
...
- Data under `examples/natural/albuquerque/` are local example inputs prepared
  ...
  PyAges transformation attribution.
- Files under `tests/data/` and `tests/golden/` are compact fixtures or
  regression values created for PyAges. They may be redistributed with the
