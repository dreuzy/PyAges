# -*- coding: utf-8 -*-
"""
Compact, didactic plots shared by installed and repository workflows.
"""
...
def plot_temporal_fit_comparison(
    ...
    for label, frame in posterior_frames.items():
        ...
        selection_mode = (
            mode if mode in {"span", "successive", "span_full"} else "single_date"
        )
        ...
        for lpm in lpm_list:
            ...
            for tracer_name, tracer_df in concentration_dict.items():
                ...
                if tracer_name not in source_predictions:
                    ...
                    source_predictions[tracer_name] = (
                        prev_dates,
                        np.vstack([prev_values, values]),
                    )
    ...
    for source_index, (_label, source_predictions) in enumerate(predictions.items()):
        ...
        legend_handles.append(
            Line2D(
                [], [], color=style["line"], linewidth=2.4, label=style["line_label"]
            )
        )
    ...
    for ax, tracer_name in zip(axs.flatten(), tracer_names):
        ...
        if highlight_array.size:
            observed_dates = pd.to_numeric(observed["date"], errors="coerce").to_numpy(
                dtype=float
            )
            highlight_mask = np.any(
                np.isclose(
    ...
    if not highlighted_any:
        legend_items = [
            handle for handle in legend_handles if handle.get_label() != highlight_label
        ]
...
def plot_objective_summary(
    ...
    for ax_index, ((xname, yname), ax) in enumerate(zip(pairs, axs)):
        if yname == objective_col:
            ...
            for method_index, (method_name, result) in enumerate(
                posterior_results.items()
            ):
                frame = _ensure_frame(result)
                color = _method_color(method_name, method_index)
                ...
                if best is not None:
                    ax.scatter(
                        ...
                        edgecolor="white",
                        linewidth=0.8,
                        label=f"{method_name} posterior best"
                        if ax_index == 0
                        else None,
                        zorder=5,
                    )
            if (
                ...
                and nearest_reference_row is not None
            ):
                ax.scatter(
