...
class ParamSysSampling:
    ...
    def plist(self):
        ...
        if self.__np == 1:
            ...
            return [
                [i, j, k] for i in self.__p[0] for j in self.__p[1] for k in self.__p[2]
            ]
        elif self.__np == 4:
            return [
                [i, j, k, fourth]
    ...
    def __pindices(self):
        ...
        if self.__np == 1:
            ...
            return [
                [i, j] for i in range(len(self.__p[0])) for j in range(len(self.__p[1]))
            ]
        elif self.__np == 3:
            return [
                [i, j, k]
    ...
    def __display_1D(
        self, Xp, Z, display_options, name_fig, xlabel="", lpm_results=None
    ):
        """
        Dislay 1D Z=f(Xp)
    ...
    def display_grid_data(
        ...
        if self.__np == 1:
            ...
            self.__display_2D(
                ...
                display_options,
                name_fig,
                xlabel=self.__pnames[index0],
                ylabel=self.__pnames[index1],
                lpm_results=lpm_results,
            )
        elif self.__np == 3:
            ...
            for k0 in range(self.__np - 1):
                ...
                self.__display_2D(
                    ...
                    display_options,
                    name_fig + "_" + str(k0),
                    xlabel=self.__pnames[k0],
                    ylabel=self.__pnames[k1],
                )
...
class SystematicSampling:
    ...
    def __init__(
        ...
        self.__psystsampling = ParamSysSampling(
            pmin, pmax, self.__nmodels, [*self.__lpm.p]
        )
    ...
    def display_concentrations_with_data(self, imax=10):
        ...
        for co, _ in zip(comb, range(imax)):
            ...
            # Displays Model
            plt.scatter(
                self.__concentrations.iloc[:, co[0]],
                self.__concentrations.iloc[:, co[1]],
                marker="+",
                c="b",
                s=10,
            )
            ...
            self.display.figure_close_fx(
                "c_reach" + tracer_name_0 + "_" + tracer_name_1
            )
    ...
    def output(self):
        ...
        """
        # Parameters
        parameters_path = os.path.join(self.display.directory, "parameters.txt")
        with open(parameters_path, "w", encoding="utf-8") as stream:
            stream.write("date\t")
            stream.write(str(self.__date))
            stream.write("\n")
            self.__lpm.write_name(stream)
            self.__tracers.write_name(stream)
            stream.write("nmodels\t")
            stream.write(str(self.__nmodels))
            stream.write("\n")
    ...
    def objective_function_build(self):
        ...
        for colmod, idat in zip(dfmodel.transpose().iterrows(), range(dfdata.shape[0])):
            ...
            ojf = ojf + L2_norm_diff(
                dfdata.loc[idat]["concentration"] * np.ones(dfmodel.shape[0]),
                colmod[1].to_numpy(),
                dfdata.loc[idat]["error"],
            )
    ...
    def objective_function_display(self, lpm_results=None):
        ...
        [p_grid, obj_function_grid] = self.__psystsampling.grid_data(
            self.__obj_function["log-ojf"]
        )
...
def test_reachconc(lpm, tracer_names, display_options, nmodels=1000, date=2010):
    ...
    cr = SystematicSampling(
        lpm,
        tracer_names,
        date=[date] * len(tracer_names),
        nmodels=nmodels,
        display_options=display_options_cr,
        reachconc=True,
    )
