...
from scripts.article import reproduce_article
from scripts.article import run_ploemeur_shifted_exponential_final as ploemeur_shifted
...
def test_fresh_campaign_rebases_every_generated_article_artifact(tmp_path):
    ...
    assert by_id["figure3_pdf"] == tmp_path / "holten_h4/figure3_holten_final.pdf"
    assert by_id["figure4_pdf"] == (
        tmp_path / "ploemeur_shifted_exponential/figure4_ploemeur_final.pdf"
    )
...
def test_stage_entrypoint_probe_reports_transitive_import_failures():
    stages = {
        "valid": reproduce_article.Stage(
            "valid", (reproduce_article.sys.executable, "-m", "json"), ()
        ),
        "missing": reproduce_article.Stage(
            "missing",
            (
                reproduce_article.sys.executable,
                "-m",
                "pyages.module_that_does_not_exist",
            ),
            (),
        ),
    }

    modules, errors = reproduce_article._check_stage_entrypoints(
        stages, ("valid", "missing")
    )

    assert modules == {
        "valid": "json",
        "missing": "pyages.module_that_does_not_exist",
    }
    assert len(errors) == 1
    assert errors[0].startswith(
        "stage missing entry point 'pyages.module_that_does_not_exist' "
        "is not importable: ModuleNotFoundError:"
    )
