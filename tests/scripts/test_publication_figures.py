...
from article.common import postprocess_existing
from scripts import run_final_holten_h4 as holten
from scripts import run_final_shifted_exponential as shifted
from scripts import run_holten_prior_robustness as prior
from scripts import run_ploemeur_shifted_exponential_final as ploemeur
...
def test_figure2_exports_three_physical_widths_with_editorial_labels(
    ...
    assert set(by_stem) == {
        ...
        "figure2_shifted_exponential_110mm",
        "figure2_shifted_exponential_100mm",
        "figure2_shifted_exponential_final",
    }
    for width in (100, 110, 120):
        actual = by_stem[f"figure2_shifted_exponential_{width}mm"]["size"][0]
        assert actual == pytest.approx(width / 25.4)
    all_labels = " ".join(by_stem["figure2_shifted_exponential_110mm"]["labels"])
...
def test_prior_sensitivity_uses_explicit_prior_names(monkeypatch, tmp_path):
    ...
    assert [item["stem"] for item in exports] == [
        "figureC1_holten_prior_sensitivity",
        "holten_prior_robustness_posteriors",
    ]
...
def test_figure4_uses_six_panels_and_only_calibration_terminology(
    ...
    assert [item["stem"] for item in exports] == [
        "figure4_ploemeur_final",
        "figure4_ploemeur_shiftedexp_final",
    ]
    final = exports[0]
    assert final["size"][0] == pytest.approx(165 / 25.4)
    combined = " ".join(final["texts"] + final["labels"])
...
def test_figure1_source_declares_readable_sans_serif_typography():
    source = (
        Path(__file__).resolve().parents[2] / "docs" / "figures" / "figure1_overview.md"
