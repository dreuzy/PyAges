...
class CalibrationExploration:
    ...
    def objective_function_build(self): 
        ...
        for colmod, idat in zip(dfmodel.transpose().iterrows(), range(dfdata.shape[0])):
            ...
            ojf=ojf+objective_function(dfdata.loc[idat]['concentration']*np.ones(dfmodel.shape[0]),
                                              colmod[1].to_numpy(),dfdata.loc[idat]['error'])
        # Sqrt of mean quadratic differences divided by error
        ojf=0.5*np.log(ojf)
        # Storage in a dataframe
        self.__obj_function = pd.DataFrame(np.hstack((np.asmatrix(self.__psystsampling.plist()),np.transpose([ojf]))),
