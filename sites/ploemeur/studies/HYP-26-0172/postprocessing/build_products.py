...
def latest_scenario_outputs(workflow: Path) -> list[tuple[Path, re.Match[str]]]:
    ...
    for scenario in workflow.iterdir() if workflow.is_dir() else []:
        ...
        if timestamps:
            outputs.append((max(timestamps, key=lambda path: path.stat().st_mtime), match))
    return outputs
...
def collect_statistics(root: Path) -> pd.DataFrame:
    ...
    for run_dir in run_directories(root):
        for output, match in latest_scenario_outputs(run_dir / "workflow"):
            ...
            for stats_file in sorted(output.glob("*_????_????/*/Metropolis_Hastings/distributions_stats.txt")):
                model_dir = stats_file.parents[1]
                case_dir = model_dir.parent
                case_match = re.fullmatch(r"(?P<well>.+)_(?P<start>\d{4})_(?P<end>\d{4})", case_dir.name)
                if not case_match:
                    continue
                ...
                for column in ("quart10", "quart25", "median", "quart75", "quart90", "mean"):
    ...
    if not records:
        raise FileNotFoundError(f"No *_stats_quantiles.txt files found below {root / 'runs'}")
...
def save_derived_tables(stats: pd.DataFrame, derived: Path) -> dict[str, Path]:
    ...
    definitions = {
        ...
        "figure6_median_transit_times.csv": stats[
            (stats["experiment_id"].str.startswith("regime_") | stats["experiment_id"].str.startswith("main_"))
            & stats["mode"].eq("successive_with_prior")
            & stats["lpm"].eq("exp_shifted")
            & stats["relative_error"].eq(0.2)
        ],
        "figureA1_error_sensitivity.csv": stats[
            (stats["experiment_id"].str.startswith("error_") | stats["experiment_id"].str.startswith("main_"))
            & stats["mode"].eq("successive_with_prior")
        ],
...
def collect_diagnostics(root: Path, derived: Path) -> Path:
    records = []
    for run_dir in run_directories(root):
        for result_file in (run_dir / "workflow").glob("**/Metropolis_Hastings/results_calibration.txt"):
            frame = pd.read_csv(result_file, sep="\t")
            values = dict(zip(frame.iloc[:, 0], frame.iloc[:, 1])) if len(frame.columns) >= 2 else {}
            stats_file = result_file.with_name("distributions_stats.txt")
            finite = True
            if stats_file.is_file():
                samples = pd.read_csv(stats_file, sep="\t")
                finite = bool(np.isfinite(samples.select_dtypes(include="number").to_numpy()).all())
            records.append(
                {
                    "experiment_id": run_dir.name,
                    "case": str(result_file.parent.parent.relative_to(run_dir / "workflow")),
                    "success_rate": pd.to_numeric(values.get("success_rate"), errors="coerce"),
                    "finite_posterior": finite,
                }
            )
    path = derived / "mcmc_diagnostics.csv"
    pd.DataFrame.from_records(records, columns=["experiment_id", "case", "success_rate", "finite_posterior"]).to_csv(path, index=False)
    return path
...
def plot_figure4(frame: pd.DataFrame, figures: Path) -> list[Path]:
    ...
    for ax, well in zip(axes, ("F11", "F09")):
        ...
        for mode, color, label in (
            ...
            ax.errorbar(data["date"], data["median_mean"], yerr=data["median_std"], fmt="o", capsize=3, color=color, label=label)
...
def plot_figure5(frame: pd.DataFrame, figures: Path) -> list[Path]:
    ...
    for ax, well in zip(axes, ("F11", "F09")):
        subset = frame[frame["well"].eq(well)]
        for model, label in (("exp_shifted", "Shifted exponential"), ("ig_shifted", "Shifted inverse Gaussian")):
            data = subset[subset["lpm"].eq(model)].sort_values("date")
            ax.plot(data["date"], data["median_mean"], "o--", color=MODEL_COLORS[model], label=label)
        ax.set_title(well, loc="left", fontweight="bold")
        ax.grid(alpha=0.25)
    axes[0].legend(frameon=False)
    axes[-1].set_xlabel("Date")
...
def plot_figure6(frame: pd.DataFrame, figures: Path, allow_partial: bool = False) -> list[Path]:
    if frame.empty:
        return []
    frame = frame.copy()
    ...
    if broken:
        fig, (top, bottom) = plt.subplots(2, 1, sharex=True, figsize=(8, 5), gridspec_kw={"height_ratios": [4, 1], "hspace": 0.05})
        axes = (top, bottom)
    else:
    ...
    for well, color in WELL_COLORS.items():
        ...
        if data.empty:
            continue
        for ax in axes:
            ax.errorbar(data["date"], data["median_mean"], yerr=data["median_std"], fmt="o--", capsize=3, color=color, label=well)
    if broken:
        top.set_ylim(max(25, high["median_mean"].min() - 8), high["median_mean"].max() + 8)
...
def plot_figure_a1(frame: pd.DataFrame, figures: Path) -> list[Path]:
    ...
    for ax, well in zip(axes, ("F11", "F09")):
        subset = frame[frame["well"].eq(well)]
        grouped = subset.groupby(["relative_error", "lpm"], as_index=False)["median_mean"].mean()
        for model, data in grouped.groupby("lpm"):
            ax.plot(100 * data["relative_error"], data["median_mean"], "o-", label=model)
        ax.set_title(well, loc="left", fontweight="bold"); ax.grid(alpha=0.25)
    axes[0].legend(frameon=False)
...
def _prediction_file(output: Path, well: str, window: tuple[int, int], model: str = "exp_shifted") -> Path | None:
    path = output / f"{well}_{window[0]}_{window[1]}" / model / "Metropolis_Hastings" / "concentrations_all_models.txt"
...
def plot_figure3(root: Path, figures: Path, window: tuple[int, int] = (2018, 2019)) -> list[Path]:
    ...
    for well in ("F09", "F11"):
        ...
        if not independent or not conditioned or not full:
            return []
        full_cases = sorted(full.glob(f"{well}_????_????"), key=lambda path: int(path.name.rsplit("_", 1)[1]) - int(path.name.rsplit("_", 2)[1]), reverse=True)
        if not full_cases:
            return []
        files = {
            "independent": _prediction_file(independent, well, window),
            "conditioned": _prediction_file(conditioned, well, window),
            "full": full_cases[0] / "exp_shifted" / "Metropolis_Hastings" / "concentrations_all_models.txt",
        }
        if not all(path and path.is_file() for path in files.values()):
            return []
        obs_file = independent / f"{well}_{window[0]}_{window[1]}" / "exp_shifted" / "concentrations.txt"
        payload[well] = ({name: pd.read_csv(path, sep="\t") for name, path in files.items()}, pd.read_csv(obs_file, sep="\t"))
    ...
    for row, well in enumerate(("F09", "F11")):
        ...
        for col, tracer in enumerate(tracers):
            ...
            ax.errorbar(obs["date"], obs["concentration"], yerr=obs["error"], fmt="o", color=OBSERVATIONS, markersize=3, capsize=2)
            if row == 0: ax.set_title(tracer.upper().replace("CFC", "CFC-"))
            if col == 0: ax.set_ylabel(f"{well}\nMixing ratio (pptv)")
            ax.set_xlim(2004, 2025); ax.grid(alpha=0.2)
...
def build(profile: str, allow_partial: bool = False) -> list[Path]:
    ...
    outputs += plot_figure4(pd.read_csv(tables["figure4_median_transit_times.csv"]), figures)
    outputs += plot_figure5(pd.read_csv(tables["figure5_model_comparison.csv"]), figures)
    outputs += plot_figure6(pd.read_csv(tables["figure6_median_transit_times.csv"]), figures, allow_partial)
    outputs += plot_figure_a1(pd.read_csv(tables["figureA1_error_sensitivity.csv"]), figures)
    return outputs
...
def main() -> None:
    ...
    parser.add_argument("--allow-partial", action="store_true", help="render figures from incomplete smoke runs")
