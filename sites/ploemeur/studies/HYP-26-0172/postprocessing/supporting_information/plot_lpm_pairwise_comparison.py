...
def verify(rows):
    grouped = {
        s: {r["model"]: r for r in rows if r["parameter_set"] == s} for s in SETS
    }
    with CHECKS.open(newline="", encoding="utf-8") as f:
        checks = {
            (int(r["pair"]), r["model"]): r
            for r in csv.DictReader(f)
            if int(r["pair"]) in SETS
        }
    for s, m in grouped.items():
        a, b = m[EXP], m[IG]
        if (
            a["shift"] != b["shift"]
            or abs(a["median"] - b["median"]) >= 0.01
            or abs(a["IQR"] - b["IQR"]) >= 0.01
        ):
            raise ValueError(f"Unmatched statistics for set {s}")
        for name in (EXP, IG):
            if abs(float(checks[(s, name)]["integral"]) - 1) >= 1e-3:
                raise ValueError(f"Integral failed for set {s}")


def build_model(r):
    if r["mu"] <= 0 or r["shift"] < 0:
        raise ValueError("Invalid parameter")
    if r["model"] == EXP:
        return ExponentialShiftedLpm(
            mu=r["mu"], shift=r["shift"], directory_lpm=LPM_DIR
        )
    if not isinstance(r["lambda"], float) or r["lambda"] <= 0:
...
def write_tables(rows):
    paths = (
        OUT / "LPM_pairwise_comparison_parameters.csv",
        OUT / "LPM_pairwise_comparison_parameters_SI.csv",
    )
    rounded = [
...
def make_figure(rows):
    ...
    for s in SETS:
        ...
        for name in (EXP, IG):
            ...
            if np.any(~np.isfinite(y)) or np.any(y < 0):
                raise ValueError(f"Invalid density set {s}")
            data[s][name] = y
    ...
    for ax, s, letter in zip(axs.flat, SETS, "abcd"):
        ...
        for name, color, ls in ((EXP, RED, "-"), (IG, GREEN, "--")):
            ...
            ax.fill_between(
                t, 0, y, where=(t >= r["Q1"]) & (t <= r["Q3"]), color=color, alpha=0.12
            )
        median = (a["median"] + b["median"]) / 2
        iqr = (a["IQR"] + b["IQR"]) / 2
        ax.axvline(median, color=".45", ls=":", lw=1.1)
        ax.set_title(
            f"({letter}) Parameter set {s}\nShift = {a['shift']:g} y; median = {median:.1f} y; IQR = {iqr:.1f} y"
        )
        ax.set_xlim(0, 160)
        ax.set_ylim(0, ymax * 1.05)
        ax.tick_params(direction="out", width=1)
    fig.supxlabel("Transit time (years)", y=0.105, fontsize=9.1)
    fig.supylabel("Probability density (year⁻¹)", x=0.025, fontsize=9.1)
    handles = [
        Line2D([], [], color=RED, lw=1.7, label="Shifted exponential"),
    ...
    fig.subplots_adjust(
        left=0.11, right=0.98, top=0.94, bottom=0.18, hspace=0.34, wspace=0.12
    )
    paths = (
        OUT / "LPM_pairwise_comparison_4panels.pdf",
        OUT / "LPM_pairwise_comparison_4panels.png",
    ...
    fig.savefig(
        paths[2], dpi=600, bbox_inches="tight", pil_kwargs={"compression": "tiff_lzw"}
    )
    plt.close(fig)
...
def main():
    ...
    verify(rows)
    for p in (*make_figure(rows), *write_tables(rows)):
        print(p)
