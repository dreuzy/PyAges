...
from validation.tracerlpm.benchmark.scripts.generate_inversion_pilot import expanded_cases, generate
...
def test_emm_inversion_pilot_recovers_truth(tmp_path: Path):
    ...
    result = invert(observation_dir=observations, result_dir=tmp_path / "results")["cases"][0]
...
def test_epm_inversion_pilot_recovers_both_parameters(tmp_path: Path):
    ...
    results = invert(observation_dir=observations, result_dir=tmp_path / "results")["cases"]
...
def test_noisy_observations_are_reproducible(tmp_path: Path):
    config = Path("validation/tracerlpm/benchmark/configs/inversion-noisy-campaign.yaml")
    first_dir, second_dir = tmp_path / "first", tmp_path / "second"
    first, second = generate(config_path=config, output_dir=first_dir), generate(
        config_path=config, output_dir=second_dir
    )
    assert [case["sha256"] for case in first["cases"]] == [case["sha256"] for case in second["cases"]]
    csv_text = (first_dir / "inversion-epm-tau20-r2-noise01-seed101.csv").read_text(encoding="utf-8")
...
def test_robustness_matrices_expand_to_expected_unique_cases():
    ...
    epm_widths = {
        ...
        for case in width_cases if case["model"] == "EPM"
    }
    dm_widths = {
