...
def expanded_cases(config: dict) -> list[dict]:
    ...
    for matrix in config.get("case_matrices", []):
        ...
        for tau, shape, noise, seed in itertools.product(
            matrix["tau_values"],
            secondary["values"],
            matrix["noise_relative_standard_deviations"],
            matrix["seeds"],
        ):
            tau = float(tau)
            shape = float(shape)
...
def _chronicle(path: Path) -> tuple[np.ndarray, np.ndarray]:
    ...
    return data["date"].to_numpy(dtype=float), data["concentration"].to_numpy(
        dtype=float
    )
...
def generate(config_path: Path = DEFAULT_CONFIG, output_dir: Path = OUTPUT_DIR) -> dict:
    ...
    for case in expanded_cases(config):
        ...
        rng = (
            np.random.default_rng(int(noise["seed"]))
            if noise["kind"] != "none"
            else None
        )
        rows = []
        for tracer in config["tracers"]:
            ...
            def input_function(year, dates=dates, concentrations=concentrations):
                return np.interp(
                    year, dates, concentrations, left=0.0, right=concentrations[-1]
                )
            ...
            value, covered_mass = forward(
                ...
                observation_year,
                input_function,
                observation_year - dates[0],
                observation_year - dates,
            )
            noise_fraction = (
            ...
            rows.append(
                {
                    ...
                    "tracer": tracer["name"],
                    "unit": tracer["unit"],
                    "observation_year": f"{observation_year:.12f}",
                    "model": case["model"],
                    "true_parameters": json.dumps(
                        case["true_parameters"], sort_keys=True
                    ),
                    ...
                    "recharge_sha256": hashlib.sha256(source.read_bytes())
                    .hexdigest()
                    .upper(),
                    ...
                    "noise_relative_standard_deviation": (
                        ""
                        if rng is None
                        else f"{float(noise['relative_standard_deviation']):.12f}"
                    ),
    ...
    for tracer in [item for item in config["tracers"] if item["name"] == "cfc12"]:
        ...
        normalized.append(
            {
                "name": tracer["name"],
                "source_sha256": hashlib.sha256(source.read_bytes())
                .hexdigest()
                .upper(),
    ...
    manifest = {
        ...
        "tracerlpm_multitracer_input": {
            ...
            "sha256": hashlib.sha256(multitracer_input.read_bytes())
            .hexdigest()
            .upper(),
        },
