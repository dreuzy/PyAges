...
def summarize() -> dict:
    ...
    for case in cases:
        ...
        secondary = result["estimated_parameters"][secondary_key] - (
            1.0 if case["model"] == "EPM" else 0.0
        )
        rows.append(
            {
    ...
    for model in ("EPM", "DM"):
        ...
        models[model] = {
            ...
            "secondary": _statistics(
                np.asarray([row["secondary"] for row in selected]), secondary_truth
            ),
        }
    summary = {
    ...
    for model, data in models.items():
        for name, truth, stats in (
            ("tau", 20.0, data["tau"]),
            (data["secondary_name"], 2.0 if model == "EPM" else 0.2, data["secondary"]),
        ):
            lines.append(
                f"| {model} | {name} | {truth:.6g} | {stats['mean']:.6g} | {stats['bias']:.6g} | "
