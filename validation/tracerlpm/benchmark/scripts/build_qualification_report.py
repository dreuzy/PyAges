...
def _load_rows() -> list[dict]:
    ...
    numeric = {
        ...
        "pyages_tau",
        "pyages_secondary",
        "pyages_maximum_concentration_relative_error",
        "tracerlpm_tau",
        "tracerlpm_secondary",
...
def _fit_objective_diagnostic(rows: list[dict]) -> dict:
    ...
    for row in rows:
        pyages = json.loads(
            (GENERATED / "inversion" / row["case_id"] / "pyages-result.json").read_text(
                encoding="utf-8"
            )
        )
        tracer = json.loads(
        ...
        pyages_relative = [
            ...
            for item in pyages["concentrations"]
        ]
        tracer_relative = [
            abs(
        ...
        comparisons.append(
            {
                "case_id": row["case_id"],
                "pyages_l1": sum(pyages_relative),
                "tracerlpm_l1": sum(tracer_relative),
                "pyages_l2": sum(value * value for value in pyages_relative),
                "tracerlpm_l2": sum(value * value for value in tracer_relative),
                "pyages_tau_error": abs(row["pyages_tau"] - row["true_tau"]),
    ...
    pyages_tau_worse = [
        ...
        if item["pyages_tau_error"] > item["tracerlpm_tau_error"]
    ]
    return {
        "case_count": len(comparisons),
        "pyages_lower_l1_count": sum(
            item["pyages_l1"] < item["tracerlpm_l1"] for item in comparisons
        ),
        "pyages_lower_l2_count": sum(
            item["pyages_l2"] < item["tracerlpm_l2"] for item in comparisons
        ),
        "pyages_tau_worse_count": len(pyages_tau_worse),
        "pyages_tau_worse_but_lower_l2_count": sum(
            item["pyages_l2"] < item["tracerlpm_l2"] for item in pyages_tau_worse
        ),
        "pyages_tau_worse_but_lower_maximum_residual_count": sum(
            row["pyages_maximum_concentration_relative_error"]
            < row["tracerlpm_maximum_concentration_relative_error"]
            for row in rows
            if abs(row["pyages_tau"] - row["true_tau"])
...
def _head_to_head(rows: list[dict]) -> dict:
    ...
    for parameter in ("tau", "secondary"):
        pyages = tracerlpm = ties = 0
        for row in rows:
            truth = row[f"true_{parameter}"]
            pyages_error = abs(row[f"pyages_{parameter}"] - truth)
            tracer_error = abs(row[f"tracerlpm_{parameter}"] - truth)
            if pyages_error < tracer_error - 1e-12:
                pyages += 1
            elif tracer_error < pyages_error - 1e-12:
                tracerlpm += 1
            else:
                ties += 1
        result[parameter] = {"pyages": pyages, "tracerlpm": tracerlpm, "ties": ties}
    return result
...
def _make_figure(model_noise: list[dict], head_to_head: dict, target: Path) -> None:
    colors = {"pyages": "#0072B2", "tracerlpm": "#D55E00"}
    labels = {"pyages": "PyAges", "tracerlpm": "TracerLPM"}
    styles = {"EPM": "-", "DM": "--"}
    ...
    axis.bar(
        x - width / 2,
        [head_to_head[name]["pyages"] for name in ("tau", "secondary")],
        width,
        color=colors["pyages"],
        label="PyAges",
    )
    ...
    axis.legend()
    fig.suptitle(
        "Qualification PyAges–TracerLPM : robustesse de l’inversion", fontsize=14
    )
    fig.tight_layout()
...
def build() -> dict:
    ...
    forward = json.loads(
        (GENERATED / "pyages_comparison" / "summary.json").read_text(encoding="utf-8")
    )
    convergence = json.loads(
        (GENERATED / "pyages_convergence" / "summary.json").read_text(encoding="utf-8")
    )
    sf6 = json.loads(
    ...
    metrics = {
        ...
        "success": {
            "pyages": sum(row["pyages_success"] for row in rows),
            "tracerlpm": sum(row["tracerlpm_success"] for row in rows),
            "pyages_boundary_hits": sum(row["pyages_boundary_hit"] for row in rows),
        ...
        "pyages_convergence": convergence,
    ...
    report = f"""# Rapport de qualification scientifique ciblée PyAges–TracerLPM

## Statut et objet du document

Ce rapport de travail consolide les tests réalisés pour préparer une section de
qualification de PyAges dans un article scientifique. Il distingue explicitement
...
pas encore, à lui seul, une validation universelle de PyAges sur données naturelles.
...
1. Le calcul *forward* de PyAges converge vers une quadrature indépendante lorsque
...
2. Sans bruit, les inversions EMM, EPM et DM récupèrent les paramètres vrais. Pour
   EPM et DM, les erreurs relatives PyAges sont inférieures à 0,04 % ; TracerLPM
...
3. À 1 % de bruit et quatre traceurs, la campagne appariée de 30 réalisations donne
   des biais faibles pour PyAges : −0,031 an en EPM et −0,029 an en DM. L'ajout de
...
5. Le classement direct des outils par distance aux paramètres vrais doit rester
   descriptif : PyAges minimise une norme L2 pondérée, tandis que TracerLPM minimise
   une somme L1 d'erreurs relatives. Dans {objectives["pyages_tau_worse_count"]} cas
   où le τ PyAges est plus éloigné de la vérité, PyAges conserve pourtant le meilleur
   critère L2 dans {objectives["pyages_tau_worse_but_lower_l2_count"]} cas. Le bruit,
...
- les distributions et convolutions de PyAges reproduisent-elles une référence
  numérique indépendante ?
- les conventions de paramètres de PyAges et TracerLPM sont-elles compatibles ?
...
Pour EPM, TracerLPM expose le rapport `r`, alors que PyAges utilise
`eta = 1 + r`. La distribution exponentielle décalée de PyAges reçoit :
...
Les concentrations vraies ne sont produites ni par PyAges ni par TracerLPM. Une
...
fichiers de recharge PyAges archivés avec leur SHA-256 dans chaque observation.
...
PyAges minimise une somme de carrés de résidus normalisés :
...
| Forward indépendant | PFM, EMM, EPM, DM ; 5 entrées, 18 paramétrisations, 3 dates | 270 | Vérifie les concentrations PyAges contre une quadrature séparée |
| Convergence PyAges | résolutions 100, 200, 500, 1000, 2000 | 5 niveaux | Sépare erreur de discrétisation et erreur de modèle |
...
| Monte-Carlo PyAges | EPM et DM, trois CFC, 1 %, 30 graines | 60 | Quantifie l'équifinalité initiale |
| Gain de SF6 | mêmes graines avec ajout de SF6 | 60 | Mesure l'information du quatrième traceur |
| Comparaison quatre traceurs | PyAges–TracerLPM, 1 %, 30 graines | 120 résultats | Compare biais et dispersion en régime favorable |
...
### 7.1 PyAges contre la référence indépendante
...
Le cas PFM avec entrée constante est identique dans PyAges, la référence et les
...
| Modèle | Paramètre | Vrai | PyAges | Erreur PyAges | TracerLPM | Erreur TracerLPM |
|---|---|---:|---:|---:|---:|---:|
...
PyAges confirme la cohérence entre les transformations de paramètres et la référence
...
- Aucun sous-ensemble de traceurs n'est testé : tous les cas utilisent les quatre
  traceurs.
- Les 480 inversions PyAges et les 480 inversions TracerLPM sont appariées sur les
  mêmes observations.
...
PyAges ajuste souvent mieux les concentrations selon L2 mais peut déplacer `tau`
...
- PyAges est plus proche de la vérité pour `tau` dans
  **{head_to_head["tau"]["pyages"]} cas**, contre
  **{head_to_head["tau"]["tracerlpm"]}** pour TracerLPM ;
- pour le second paramètre, PyAges est plus proche dans
  **{head_to_head["secondary"]["pyages"]} cas**, TracerLPM dans
  **{head_to_head["secondary"]["tracerlpm"]}**, avec
  **{head_to_head["secondary"]["ties"]} égalités** ;
- PyAges a le plus faible objectif L2 recalculé dans
  **{objectives["pyages_lower_l2_count"]}/480 cas** ;
- PyAges a la plus faible somme L1 dans seulement
  **{objectives["pyages_lower_l1_count"]}/480 cas** ;
- parmi les {objectives["pyages_tau_worse_count"]} cas où son `tau` est moins proche
  de la vérité, PyAges conserve un objectif L2 inférieur dans
  **{objectives["pyages_tau_worse_but_lower_l2_count"]} cas** et une erreur relative
  maximale de concentration inférieure dans
  **{objectives["pyages_tau_worse_but_lower_maximum_residual_count"]} cas**.
...
- Les définitions EPM et DM de PyAges sont cohérentes avec les distributions
  analytiques et les correspondances de paramètres documentées.
- Le chemin de convolution PyAges converge vers une quadrature indépendante.
- PyAges récupère les paramètres synthétiques sans bruit pour EMM, EPM et DM.
- En régime favorable à quatre traceurs et 1 % de bruit, PyAges présente un biais
...
- PyAges et TracerLPM seraient numériquement interchangeables.
...
> PyAges a été qualifié par une stratégie hiérarchique combinant des solutions
...
> PyAges et TracerLPM résultent à la fois de fonctions objectif différentes, de la
> discrétisation propre à chaque outil et de l'équifinalité du problème inverse.

Cette formulation qualifie PyAges sans présenter TracerLPM comme une vérité absolue :
...
1. **Harmoniser le critère de comparaison.** Réévaluer un sous-ensemble
   ...
   inversion PyAges sous L1 uniquement pour le diagnostic.
        ...
        json.dumps(
            {
                ...
                "pyages_success": result["success"]["pyages"],
