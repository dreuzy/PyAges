...
def _noise_realizations(noise_config: dict) -> list[tuple[str, float, int | None]]:
    ...
    result.append(
        (
            "low_deterministic",
            float(low["relative_standard_deviation"]),
            int(low["seed"]),
        )
    )
    ...
    result.extend(
        (
            f"repeated_{index + 1}",
            float(repeated["relative_standard_deviation"]),
            int(seed),
        )
...
def generate(
    ...
    forward_path: Path = DEFAULT_FORWARD_REFERENCE,
    ...
    rng_by_seed = {
        seed: np.random.default_rng(seed) for _, _, seed in regimes if seed is not None
    }
        ...
        for truth_index, truth in enumerate(truth_rows):
            ...
            for regime, relative_std, seed in regimes:
                factor = (
                    1.0
                    if seed is None
                    else 1.0 + float(rng_by_seed[seed].normal(0.0, relative_std))
                )
    ...
    manifest = {
        ...
        "forward_reference_sha256": hashlib.sha256(forward_path.read_bytes())
        .hexdigest()
        .upper(),
        "synthetic_observations_sha256": hashlib.sha256(output_path.read_bytes())
        .hexdigest()
        .upper(),
