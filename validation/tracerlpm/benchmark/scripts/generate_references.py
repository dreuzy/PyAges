...
def generate(config_path: Path = DEFAULT_CONFIG, output_path: Path = DEFAULT_OUTPUT, manifest_path: Path = DEFAULT_MANIFEST) -> dict:
        ...
        for model, parameter_grid in config["models"].items():
            for parameters in _parameter_sets(model, parameter_grid):
                for observation_year in config["observation_years"]:
                    ...
                    parameter_text = ";".join(f"{key}={float(val):.12g}" for key, val in parameters.items())
                    rows.append((input_name, model, parameter_text, float(observation_year), value, mass))
    output_path.parent.mkdir(parents=True, exist_ok=True)
    with output_path.open("w", encoding="utf-8", newline="") as stream:
        writer = csv.writer(stream, lineterminator="\n")
        writer.writerow(["input", "model", "parameters", "observation_year", "concentration", "covered_mass"])
        for row in rows:
            writer.writerow([row[0], row[1], row[2], f"{row[3]:.12f}", f"{row[4]:.12f}", f"{row[5]:.12f}"])
    manifest = {
        "campaign_id": config["campaign_id"],
        "kind": "independent_forward_reference",
        "row_count": len(rows),
        "campaign_config_sha256": hashlib.sha256(config_bytes).hexdigest().upper(),
        "forward_reference_sha256": hashlib.sha256(output_path.read_bytes()).hexdigest().upper(),
