...
"""Run the 60-case PyAges Monte-Carlo campaign in parallel worker processes."""
...
from .invert_pyages_pilot import invert
...
def _run_one(case_id: str, config_path: Path) -> dict:
    return invert(
        config_path=config_path, observation_dir=OUTPUT_DIR, case_ids={case_id}
    )["cases"][0]
...
def run(config_path: Path = CONFIG, workers: int = 4) -> dict:
    ...
    with ProcessPoolExecutor(max_workers=workers) as executor:
        futures = {
            executor.submit(_run_one, case_id, config_path): case_id
