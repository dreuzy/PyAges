...
CONFIGS = {
    ...
    "three_cfcs_plus_sf6": BENCHMARK_ROOT / "configs" / "inversion-monte-carlo-01-sf6.yaml",
}
...
def _load(path, prefix: str) -> dict[str, list[dict]]:
    ...
    for case in expanded_cases(config):
        ...
        rows[case["model"]].append({
            "seed": int(case["noise"]["seed"]), "tau": float(result["estimated_parameters"]["tau"]),
            "secondary": secondary, "source": prefix,
        })
...
def _metrics(rows: list[dict], secondary_truth: float) -> dict:
    ...
    return {
        "tau_sd": float(np.std(tau, ddof=1)), "tau_rmse": float(np.sqrt(np.mean((tau - 20.0) ** 2))),
...
def compare() -> dict:
    ...
    for model, secondary_name, truth in (("EPM", "r", 2.0), ("DM", "DP", 0.2)):
        ...
        models[model] = {"secondary_name": secondary_name, "three_cfcs": before,
                         "three_cfcs_plus_sf6": after, "relative_reductions": reductions}
    summary = {"paired_seeds": list(range(201, 231)), "noise_relative_sd": 0.01, "models": models}
    OUTPUT.mkdir(parents=True, exist_ok=True)
    (OUTPUT / "summary.json").write_text(json.dumps(summary, indent=2) + "\n", encoding="utf-8")
    lines = ["# Gain d'information du SF6 — 30 réalisations appariées", "",
             "| Modèle | Mesure | 3 CFC | 3 CFC + SF6 | Réduction |",
             "|---|---|---:|---:|---:|"]
    labels = {"tau_sd": "écart type tau", "tau_rmse": "RMSE tau",
              "secondary_sd": "écart type paramètre 2", "secondary_rmse": "RMSE paramètre 2"}
    for model, data in models.items():
        for key, label in labels.items():
            lines.append(f"| {model} | {label} | {data['three_cfcs'][key]:.6g} | "
                         f"{data['three_cfcs_plus_sf6'][key]:.6g} | {data['relative_reductions'][key]:.1%} |")
        lines.append(f"| {model} | corrélation tau–{data['secondary_name']} | "
