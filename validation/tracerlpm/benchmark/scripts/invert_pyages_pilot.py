...
"""Invert no-noise one- and two-parameter pilots with current PyAges."""
...
def invert(
    ...
    for case in expanded_cases(config):
        ...
        if case["model"] not in MODEL_NAMES:
            raise ValueError("Le pilote PyAges accepte EMM, EPM ou DM")
...
def _invert_case(config: dict, case: dict, observation_dir: Path) -> dict:
    ...
    model = build_lpm(MODEL_NAMES[case["model"]])
...
def _write_case_result(summary: dict, target: Path) -> None:
    ...
    (target / "pyages-result.json").write_text(
        json.dumps(summary, indent=2) + "\n", encoding="utf-8"
    )
    rows = summary["concentrations"]
    with (target / "pyages-concentrations.csv").open(
