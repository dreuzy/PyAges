...
def study(config_path: Path = DEFAULT_CONFIG, observation_dir: Path = OUTPUT_DIR,
          output_dir: Path | None = None, model: str = "EPM") -> dict:
    ...
    with (observation_dir / f"{case['case_id']}.csv").open(encoding="utf-8", newline="") as stream:
    ...
    tau_values = np.unique(np.append(np.linspace(5.0, 60.0, 31), case["true_parameters"]["tau"]))
    secondary_name = "eta" if model == "EPM" else "DP"
    secondary_values = (
        np.unique(np.append(np.linspace(1.05, 10.0, 31), case["true_parameters"]["eta"]))
        if model == "EPM"
        else np.unique(np.append(np.geomspace(0.005, 3.0, 31), case["true_parameters"]["DP"]))
    )
    rows = []
    for tau in tau_values:
        for secondary in secondary_values:
            ...
            for observation in observations:
                ...
                predicted = forward(
                    model, {"tau": tau, secondary_name: secondary}, date,
                    lambda year, d=dates, v=values: np.interp(year, d, v, left=0.0, right=v[-1]),
                    date - dates[0], date - dates,
                )[0]
            ...
            row = {"tau": float(tau), secondary_name: float(secondary),
    ...
    with (output_dir / "objective-surface.csv").open("w", encoding="utf-8", newline="") as stream:
    ...
    summary = {
        ...
        "true_parameters": case["true_parameters"], "grid_best": best,
        "note": "Coarse independent surface; optimizer results remain the precision estimate.",
    }
    (output_dir / "surface-summary.json").write_text(json.dumps(summary, indent=2) + "\n", encoding="utf-8")
    return summary
