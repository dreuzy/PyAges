...
def compare(run_json: Path) -> dict:
    ...
    if model == "EPM":
        secondary_name, true_secondary = "r", float(case["true_parameters"]["eta"]) - 1.0
    ...
    secondary_limit = float(config["acceptance"]["maximum_secondary_parameter_relative_error"])
    parameter_rows = [
        {"parameter": "tau", "true": true_tau, "pyage": pyage_tau, "tracerlpm": tracer_tau,
         "pyage_relative_error": abs(pyage_tau-true_tau)/true_tau,
         "tracerlpm_relative_error": abs(tracer_tau-true_tau)/true_tau},
        {"parameter": secondary_name, "true": true_secondary, "pyage": pyage_secondary,
         "tracerlpm": tracer_secondary,
         "pyage_relative_error": abs(pyage_secondary-true_secondary)/true_secondary,
         "tracerlpm_relative_error": abs(tracer_secondary-true_secondary)/true_secondary},
    ]
    ...
    for alias, calculated in best_attempt["calculatedConcentrations"].items():
        ...
        concentrations.append({"tracer": ALIASES[alias], "alias": alias, "observed": observed,
                               "tracerlpm_calculated": float(calculated),
                               "tracerlpm_relative_error": abs(float(calculated)-observed)/observed})
    ...
    lines = [f"# Inversion {model} sans bruit — trois CFC", "",
             "| Paramètre | Vrai | PyAge | TracerLPM | Erreur rel. PyAge | Erreur rel. TracerLPM |",
             "|---|---:|---:|---:|---:|---:|"]
    for row in parameter_rows:
        lines.append(f"| {row['parameter']} | {row['true']:.9g} | {row['pyage']:.9g} | {row['tracerlpm']:.9g} | {row['pyage_relative_error']:.3%} | {row['tracerlpm_relative_error']:.3%} |")
    lines += ["", f"Verdicts : PyAge `{'pass' if summary['pyage_pass'] else 'investigate'}` ; TracerLPM `{'pass' if summary['tracerlpm_pass'] else 'investigate'}`.", ""]
