...
def prepare(config_path: Path = DEFAULT_CONFIG, output_path: Path = DEFAULT_OUTPUT,
            case_ids: set[str] | None = None) -> list[dict]:
    ...
    for source in expanded_cases(config):
        if case_ids is not None and source["case_id"] not in case_ids:
            continue
        with (OUTPUT_DIR / f"{source['case_id']}.csv").open(encoding="utf-8", newline="") as stream:
            observations = {row["tracer"]: float(row["observed_concentration"])
                            for row in csv.DictReader(stream)}
        epm = source["model"] == "EPM"
        secondary_key = "eta" if epm else "DP"
        secondary_offset = 1.0 if epm else 0.0
        true_model_parameter = float(source["true_parameters"][secondary_key]) - secondary_offset
        initial_pairs = [[float(value) for value in pair] for pair in source["initial_values"]]
        cases.append({
            ...
            "tracerlpm_effective_observation_year": float(config["observation_year"]),
            "model1": source["model"], "model2": source["model"],
            ...
            "input_history": {
                ...
                "before": 0.0, "after": "hold_last",
            },
            "fit": {
                "sample": tracerlpm.get("sample", "PSW-1-17/08/2004"),
                "model": source["model"],
                "observations": {target: observations[source_columns[target]]
                                 for target in target_tracers},
                "initial_ages": [pair[0] for pair in initial_pairs],
                "initial_model_parameters": [pair[1] - secondary_offset for pair in initial_pairs],
                "age_lower": float(source["bounds"]["tau"][0]),
                "age_upper": float(source["bounds"]["tau"][1]),
                "model_parameter_lower": float(source["bounds"][secondary_key][0]) - secondary_offset,
                "model_parameter_upper": float(source["bounds"][secondary_key][1]) - secondary_offset,
            },
        })
...
if __name__ == "__main__":
    ...
    print(f"{len(prepare(args.config, args.output, set(args.case_id) or None))} cas TracerLPM générés")
