...
def parse_parameters(text: str) -> dict[str, float]:
    return {item.split("=", 1)[0]: float(item.split("=", 1)[1]) for item in text.split(";")}
...
def symmetric_relative_difference(value: float, reference: float, floor: float = 1e-12) -> float:
...
def compare(
    ...
    for reference in references:
        ...
        rows.append({
            **reference,
            ...
            "pyage_parameters": ";".join(f"{key}={val:.12g}" for key, val in effective_parameters.items()),
            "pyage_concentration": value,
            "signed_difference": signed,
            "absolute_difference": abs(signed),
            "symmetric_relative_difference": symmetric_relative_difference(value, expected),
        })
    ...
    report = {
        ...
        "reference_sha256": hashlib.sha256(reference_path.read_bytes()).hexdigest().upper(),
    ...
    markdown = ["# Comparaison PyAge–référence indépendante", "", f"Statut : `{report['status']}`", "", "| Modèle | Cas | Biais | MAE | RMSE | Max absolu | Max relatif symétrique |", "|---|---:|---:|---:|---:|---:|---:|"]
    for model in MODEL_NAMES:
        item = families[model]
        markdown.append(f"| {model} | {item['case_count']} | {item['bias']:.6g} | {item['mae']:.6g} | {item['rmse']:.6g} | {item['maximum_absolute_difference']:.6g} | {item['maximum_absolute_symmetric_relative_difference']:.6g} |")
    markdown.extend(["", "Aucun verdict pass/fail n’est appliqué avant l’étude de convergence.", ""])
