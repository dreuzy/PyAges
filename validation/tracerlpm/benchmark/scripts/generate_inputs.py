...
def decimal_years(start_year: int, end_year: int) -> np.ndarray:
    ...
    return np.array(
        [year + (month - 0.5) / 12 for year in range(start_year, end_year + 1) for month in range(1, 13)],
        dtype=float,
    )
...
def build_series(config: dict) -> tuple[np.ndarray, dict[str, np.ndarray]]:
    ...
    if grid["frequency"] != "monthly" or grid["timestamp_convention"] != "middle_of_month":
    ...
    ramp = np.linspace(float(ramp_spec["start_value"]), float(ramp_spec["end_value"]), years.size)
    ...
    inside = (years >= float(pulse_spec["start_year"])) & (years < float(pulse_spec["end_year"]))
...
def generate(config_path: Path = DEFAULT_CONFIG, output_dir: Path = DEFAULT_OUTPUT, manifest_path: Path = DEFAULT_MANIFEST) -> dict:
    ...
    for name, values in series.items():
        ...
        files.append({"name": name, "path": _manifest_path(path, output_dir), "rows": int(years.size), "sha256": _sha256(path)})
    manifest = {
