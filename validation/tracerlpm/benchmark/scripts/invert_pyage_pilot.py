...
def invert(
    ...
    for case in expanded_cases(config):
        ...
        bounds = [
            tuple(float(value) for value in case["bounds"][name])
        ...
        successful_near_best = [
            ...
            and float(item.fun) <= minimum_objective + objective_tolerance
        ]
        ...
        true_parameters = {
            name: float(case["true_parameters"][name]) for name in parameter_names
        }
            ...
            rows.append(
                {
                    ...
                    "residual": float(calc - obs),
                    "normalized_residual": float((calc - obs) / scale),
                }
            )
        ...
        summary = {
            ...
            "parameter_absolute_errors": {
                name: abs(estimated_parameters[name] - true_parameters[name])
                for name in parameter_names
            },
