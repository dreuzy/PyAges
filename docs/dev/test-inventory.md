...
|---|---|---|
| Standard selection | 956 | 94 |
| Extensive opt-in | 5 | 2 |
| Core including extensive | 961 | 94 |
| TracerLPM validation | 65 | 9 |
| All documented pytest scopes | 1026 | 103 |

## Cases by area

| Area | Location | Primary type | Contract | CI scope | Modules | Cases |
|---|---|---|---|---|---|---|
| Repository-wide contracts | `tests/(root)/` | Contract / integration | Public API, metadata, manifests, paths, and repository documentation | Standard CI | 9 | 32 |
| Calibration and inference | `tests/calibration/` | Unit / scientific | Objectives, priors, proposals, parameter grids, and calibration APIs | Standard CI; selected extensive cases | 16 | 115 |
...
| Concentration handling | `tests/concentrations/` | Unit / data contract | Chronicle loading and concentration-series behavior | Standard CI | 3 | 44 |
| Configuration | `tests/config/` | Unit / contract | Validated models, runtime options, and portable path resolution | Standard CI | 3 | 36 |
...
| Input/output | `tests/data_io/` | Unit / data contract | LPM parameter parsing and serialization | Standard CI | 2 | 46 |
...
| Lumped-parameter models | `tests/lpm/` | Analytical / unit / golden | Distributions, moments, mixtures, registries, parameters, and generated values | Standard CI | 15 | 240 |
| Ploemeur field case | `tests/ploemeur/` | Field integration / golden | Preparation, configuration, reference convolution, paths, and workflow outputs | Standard CI; selected extensive cases | 8 | 53 |
| Scientific orchestration | `tests/scripts/` | Integration / reproducibility | Article campaigns, qualification scripts, and reproducible execution support | Standard CI | 12 | 65 |
| Environmental tracers | `tests/tracer/` | Scientific unit / contract | Decay, distributed inputs, tracer configuration, and public tracer behavior | Standard CI | 4 | 49 |
| Installed workflows | `tests/workflows/` | Integration / contract | Plotting runtime and single-date workflow behavior | Standard CI and package smoke test | 5 | 17 |
...
| `tests/calibration/test_calibration_problem.py` | Unit / scientific | Calibration problem within calibration and inference. | 6 | 0 |
...
| `tests/calibration/test_prior_edge_cases.py` | Unit / scientific | Prior edge cases within calibration and inference. | 15 | 0 |
...
| `tests/cli/test_cli_diagnostics.py` | Contract / integration | CLI diagnostics within command-line interface. | 14 | 0 |
...
| `tests/concentrations/test_concentration_contracts.py` | Unit / data contract | Concentration contracts within concentration handling. | 31 | 0 |
...
| `tests/config/test_config_models.py` | Unit / contract | Config models within configuration. | 28 | 0 |
| `tests/config/test_paths.py` | Unit / contract | Paths within configuration. | 2 | 0 |
| `tests/config/test_runtime.py` | Unit / contract | Runtime within configuration. | 6 | 0 |
...
| `tests/lpm/test_sample_diagnostics_plotting.py` | Analytical / unit / golden | Sample diagnostics plotting within lumped-parameter models. | 4 | 0 |
...
| `tests/ploemeur/test_ploemeur_workflow.py` | Field integration / golden | Ploemeur workflow within ploemeur field case. | 2 | 0 |
...
| `tests/scripts/test_article_campaign.py` | Integration / reproducibility | Article campaign within scientific orchestration. | 15 | 0 |
| `tests/scripts/test_article_support.py` | Integration / reproducibility | Article support within scientific orchestration. | 15 | 0 |
...
| `tests/scripts/test_publication_figures.py` | Integration / reproducibility | Publication figures within scientific orchestration. | 7 | 0 |
...
| `tests/scripts/test_zenodo_bundle.py` | Integration / reproducibility | Zenodo bundle within scientific orchestration. | 4 | 0 |
...
| `tests/test_project_metadata.py` | Contract / integration | Project metadata within repository-wide contracts. | 3 | 0 |
...
| `tests/test_result_manifest.py` | Contract / integration | Result manifest within repository-wide contracts. | 2 | 0 |
...
| `tests/workflows/test_plotting_runtime_additional.py` | Integration / contract | Plotting runtime additional within installed workflows. | 4 | 0 |
| `tests/workflows/test_single_date_components.py` | Integration / contract | Single date components within installed workflows. | 8 | 0 |
| `tests/workflows/test_single_date_workflow.py` | Integration / contract | Single date workflow within installed workflows. | 2 | 0 |
