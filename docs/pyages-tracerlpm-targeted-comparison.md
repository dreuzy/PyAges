# Protocole et état — qualification ciblée de PyAges par TracerLPM
...
**480 cas appariés** EPM/DM exécutés par PyAges et TracerLPM. Les sorties brutes
...
Vérifier sur des cas reproductibles que PyAges et TracerLPM sont compatibles
...
- PyAges interprète désormais `mu` comme moyenne et `sigma` comme écart type de
...
| PyAges | TracerLPM | Fonction testée |
|---|---|---|
...
```text
PyAges mu = TracerLPM mean_age = tau
```
...
de PyAges est `eta = (x+x*)/x`, d’où la conversion indispensable :
...
Il n’existe plus d’anomalie mathématique connue dans le DM de PyAges. Les points
...
`r = 0.5, 2, 9`, tandis que PyAges reçoit `eta = 1.5, 3, 10`.

## 7. Architecture proposée

Le runner reste un adaptateur Excel autonome ; le cœur PyAges ne connaîtra ni
...
configuration TracerLPM ne sera dispersé dans `pyages/`, `tests/` ou les exemples
généraux. Seule une correction scientifique générique, démontrée indépendamment
du benchmark, pourra modifier le cœur PyAges et devra alors posséder ses propres
...
semestrielle. Les écarts PyAges concentrés aux très faibles âges restent consignés
comme effets de résolution ; conformément au périmètre décidé, ils ne déclenchent
pas de modification supplémentaire de PyAges à ce stade.

### Phase 4 — inversion synthétique et identifiabilité

- inverser d’abord les observations sans bruit issues de paramètres vrais ;
- comparer paramètres vrais, estimations PyAges et estimations TracerLPM ;
...
`19.9982589 ans` avec PyAges et `19.9338202 ans` avec TracerLPM. Les erreurs de
...
EPM retrouve `(tau, r)=(20.0000005, 2.0000001)` dans PyAges et
`(19.7464101, 1.9624315)` dans TracerLPM, pour une vérité `(20, 2)`. DM retrouve
`(tau, DP)=(19.9995879, 0.2000672)` dans PyAges et
...
inversions PyAges convergent et leurs biais/RMSE sont consignés dans
...
`tau/r = 1.976/0.421` pour PyAges et `1.836/0.445` pour TracerLPM en EPM, puis
...
**Extension Monte-Carlo du 18 août 2026 :** 30 réalisations PyAges par modèle à
...
- mesurer séparément discrétisation temporelle et quadrature PyAges ;
...
**État au 18 août 2026 : terminé pour le périmètre synthétique.** PyAges converge
...
PyAges sont inférieures à 0,04 % pour EPM et DM. Les deux outils terminent les
...
PyAges, somme L1 relative dans TracerLPM), les trois CFC sont corrélés, seulement
...
PyAges–TracerLPM, fonction objectif, convergence, initialisation, bornes actives,
...
- [Migration scientifique PyAges](scientific-migration-ig-decay.md)
